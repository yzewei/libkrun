...
fn write_krun_config(root_dir: &Path, test_case: &str) -> anyhow::Result<()> {
    let config_path = root_dir.join(".krun_config.json");
    let config = format!(
...
pub fn setup_fs_and_enter(ctx: u32, test_setup: TestSetup) -> anyhow::Result<()> {
    ...
    write_krun_config(&root_dir, &test_setup.test_case)?;
    unsafe {
