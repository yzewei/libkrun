...
/*
 * IOCSR IPI emulation notes:
 * The userspace IPI path (IPI_STATUS/EN/SET/CLEAR/SEND + KVM_INTERRUPT injection)
 * is intentionally disabled for now.
 * Current upstreamable LoongArch support is single-vCPU only, so this path is
 * not required in this series.
 *
 * Constants and implementation kept here as reference for future SMP re-enable:
 *
...
pub const LOONGARCH_IOCSR_MBUF_SEND: u64 = 0x1048; //LOONGARCH_IOCSR_IPI_MAIL_SEND
...
impl LoongArchIocsrState {
    ...
    /*
     * Userspace IPI emulation is disabled in single-vCPU mode.
     * Keep previous implementation around for future SMP re-enable.
     */
    ...
    // Userspace IPI path currently disabled; no vcpu-fd registry needed.
}
